...
function CheckoutForm({ checkoutData, userId }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
  type="submit"
  className="w-full bg-yellow-500 text-white px-6 py-3 rounded-lg hover:bg-yellow-600 transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        {clientSecret && (
          <Elements stripe={stripePromise} options={{ clientSecret }}>
            <CheckoutForm checkoutData={checkoutData} userId={userId} />
          </Elements>
        )}
      </div>
    </div>
